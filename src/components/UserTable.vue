    ...
    <div v-show="!loading && !error">
      <form class="mb-3 flex">
        ...
        <button v-if="query" class="ml-1 bg-gray-100 px-3 rounded-xl">
          <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
               width="15" height="15" viewBox="0 0 590.556 590.556" style="enable-background:new 0 0 590.556 590.556;"
               xml:space="preserve">
      ...
      <table class="w-full">
        ...
        <tr
          ...
          <td class="p-2">
            <div class="rounded-full overflow-hidden bg-gray-10 w-12 h-12">
...
export default {
  ...
  methods: {
    ...
    async getUsers() {
      this.loading = true
      try {
        const {data: {data, total_pages}} = await axios.get('https://reqres.in/api/users', {
          params: {
            delay: 1,
            ...this.params
          }
    ...
    async getRoles() {
      try {
        const {data} = await GET_ROLES({delay: 300})
        this.roles = data
